...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, room:str, id:int):
        await websocket.accept()
        self.active_connections.append({"websocket": websocket, "room":room, "id": id})
        print('CONNECTED', id)

    def disconnect(self, websocket: WebSocket, room:str, id:int):
    ...
    async def broadcast(self, data: str, room: str, id: int):
...
async def websocket_endpoint(websocket: WebSocket, room_id:str, client_id: int):
    ...
    try:
        while True:
            ...
            await manager.broadcast(f"Client #{ client_id} says: {data}", room_id, client_id)
    except WebSocketDisconnect:
        manager.disconnect(websocket, room_id, client_id)
        await manager.broadcast(f"Client #{ client_id} left chat",room_id, client_id )